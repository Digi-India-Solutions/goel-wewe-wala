...
const createRecord = async (req, res) => {
  ...
  try {
    ...
    await transporter.sendMail({
      ...
      to: email,
      subject: "Welcome to Goel Mewewale!",
...
const login = async (req, res) => {
  ...
  try {
    ...
    res.cookie("token", token, {
      ...
      domain: ".goelmewewale.com", // Allow across main domain and subdomains
    });
...
const logout = async (req, res) => {
  try {
    ...
    res.clearCookie("token", {
      ...
      secure: true, // Ensure cookie is sent over HTTPS
      sameSite: "None", // Allow cross-site usage (if needed)
      domain: ".goelmewewale.com", // Match your domain
    });
...
const getPasswordResetEmailTemplate = (name, resetLink) => `
    <div style="font-family: Arial, sans-serif; margin: 0; padding: 0; width: 100%; background-color: #F7F7F7;">
        <table style="max-width: 600px; margin: 50px auto; background: #FFFFFF; border: 1px solid #E8E8E8; border-radius: 10px;">
            <tr>
                <td style="background: #8B4513; padding: 20px; text-align: center;">
                    <h1 style="color: #FFFFFF; font-size: 24px;">Password Reset Request - Goelmewewale</h1>
                </td>
            </tr>
            <tr>
                <td style="padding: 20px;">
                    <h2 style="color: #333333; font-size: 20px; margin-bottom: 10px;">Hello ${name},</h2>
                    <p style="color: #555555; font-size: 16px; line-height: 1.5; margin: 0;">
                        We received a request to reset your password for your Goelmewewale account. If you requested this change, click the link below to reset your password:
                    </p>
                    <p style="color: #555555; font-size: 16px; line-height: 1.5; margin: 20px 0;">
                        <a href="${resetLink}" style="color: #8B4513; text-decoration: none; font-size: 16px; font-weight: bold;">Reset Your Goelmewewale Password</a>
                    </p>
                    <p style="color: #555555; font-size: 14px; line-height: 1.5; margin: 0;">
            ...
            <tr>
                <td style="padding: 20px;">
                    <p style="color: #555555; font-size: 14px; line-height: 1.5; margin: 0;">
                        ...
                        <strong>The Goelmewewale Team</strong>
                    </p>
                    <p style="color: #555555; font-size: 12px; line-height: 1.5; margin-top: 10px;">
                        Explore our range of fresh dry fruits and seeds at <a href="goelmewewale.com" style="color: #8B4513; text-decoration: none;">Goelmewewale.com</a>
                    </p>
                </td>
            </tr>
        </table>
...
const forgotPassword = async (req, res) => {
  ...
  try {
    ...
    const resetLink = `https://goelmewewale.com/reset-password/${resetToken}`;
