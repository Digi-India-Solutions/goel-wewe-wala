...
const Checkout = () => {
  ...
  useEffect(() => {
    ...
    const getApiData = async () => {
      try {
        const res = await axios.get(`https://api.goelmewewale.com/api/get-user/${userId}`);
        if (res.status === 200) {
  ...
  const handleConfirmOrder = async (event) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        try {
          const res = await axios.post("https://api.goelmewewale.com/api/checkout", checkoutData);
          if (res.status === 201) {
