      ...
      for (const product of order.products) {
        try {
          const response = await axios.get(
            `https://api.goelmewewale.com/api/single-product/${product.productId}`
          );
          const productDetails = response.data.product;

          const productInfo = productDetails.productInfo.find(
            (info) => info.productweight === product.weight
          );
          const tax = productInfo?.tax || 0;
          details.push({ ...product, tax });
        } catch (error) {
          console.error(
            `Error fetching product details for ID: ${product.productId}`,
            error
          );
...
const EditOrder = () => {
  ...
  useEffect(() => {
    const fetchOrder = async () => {
      try {
        const response = await axios.get(
          `https://api.goelmewewale.com/api/single-order-data/${orderId}`
        );
        if (response.data.success) {
          ...
          setOrderData({
            ...
            user_details: response.data.data.shippingAddress,
          });
        } else {
  ...
  const handleUpdate = async () => {
    try {
      const response = await axios.put(
        `https://api.goelmewewale.com/api/update-order/${orderId}`,
        {
          orderStatus,
          paymentStatus,
        }
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const response = await axios.post(
        "https://api.goelmewewale.com/api/login-via-shiprocket",
        payload,
        {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "https://api.goelmewewale.com/api/shiped-order-shiprocket",
        orderData,
        {
          headers: { "Content-Type": "application/json" },
        }
      ...
      <div className="container mt-4">
                  ...
                  <div className="card-body">
                    {order.products.map((product, index) => (
                      <div className="mb-3" key={index}>
                        ...
                        <p className="mb-0">
                          Subtotal Price: ₹{product.price * product.quantity}
                        </p>
                      </div>
                    ))}
                  </div>
        ...
        {step === 2 || step === 3 ? (
          ...
          <div className="">
            ...
            <button className="btn btn-primary mx-2 my-3" onClick={handleLogin}>
              Ready To Ship
            </button>
          </div>
        )}
